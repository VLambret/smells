...
fn do_internal_analysis(
    ...
    let mut tree_of_analyses =
        TreeOfAnalyses::new(root_id.clone(), hashmap! {root_id => root_analysis });

    // SMELLS : wrong level of abstraction (parents)
    // SMELLS : for into for
    for file in file_explorer.discover() {
        ...
        let file_analysis = create_and_push_file_analysis_into_tree_of_analyses(
            &metrics,
            &mut tree_of_analyses.analyses,
            &file,
            &mut last_parent_of_file_id,
    ...
    convert_analyses_in_tree_of_analyses_to_final_analysis(
        tree_of_analyses.analyses.clone(),
        tree_of_analyses.get_root(),
    )
}
...
fn add_parent_analysis_to_tree_of_analyses(
    ...
    if analyses_in_tree_of_analyses
        .get_mut(&parent_analysis_id)
        .is_none()
    {
        ...
        connect_grand_father_with_parent_in_tree_of_analyses(
            analyses_in_tree_of_analyses,
            &parent_analysis_id,
            &mut parent_analysis,
        );
        analyses_in_tree_of_analyses.insert(parent_analysis_id.clone(), parent_analysis);
    }
...
fn connect_grand_father_with_parent_in_tree_of_analyses(
    ...
    if let Some(grand_parent) = PathBuf::from(parent_analysis.file_name.clone()).parent() {
        ...
        if let Some(grand_parent_analysis) = analyses_in_tree_of_analyses.get_mut(&grand_parent_id)
        {
...
// SMELLS: do 2 different things
fn create_and_push_file_analysis_into_tree_of_analyses(
    metrics: &Vec<Box<dyn IMetric>>,
    analyses_in_tree_of_analyses: &mut HashMap<AnalysisInTreeId, AnalysisInTree>,
    file: &Path,
    last_parent_of_file_id: &mut AnalysisInTreeId,
    ...
    let last_parent_folder_content = analyses_in_tree_of_analyses
        .get_mut(last_parent_of_file_id)
        .unwrap()
        .folder_content_id
        .get_or_insert(vec![]);
    last_parent_folder_content.push(file_id);
    file_analysis
}
...
fn propagate_file_scores_to_parents_analysis(
    ...
    add_file_metrics_to_parent_analysis(
        analyses_in_tree_of_analyses,
        parent_id,
...
fn add_file_metrics_to_parent_analysis(
    analyses_in_tree_of_analyses: &mut HashMap<AnalysisInTreeId, AnalysisInTree>,
...
fn convert_analyses_in_tree_of_analyses_to_final_analysis(
...
mod tests {
    ...
    fn analyse_internal_with_one_5_lines_file() {
        ...
        let mut expected_metrics = BTreeMap::new();
        expected_metrics.insert("lines_count", Some(MetricsValueType::Score(5)));
